...
@end
@implementation ResolveFecherNew

+ (ResolveFecherNew *)sharedResolveFecherNew {
  static ResolveFecherNew *_sharedResolveFecher = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    _sharedResolveFecher = [[ResolveFecherNew alloc] init];
  });
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _parameters[@"uid"] = [OpenUDID value];//@"99000567710378";
...
- (void)resolveFectcherWithTelephoneNumber:(NSString *)telephoneNumber completionHandler:(void (^)( ResolveItemNew *resolveItem, NSError *error) )completionHandler {
    ...
    if (![ResolveFecherNew canResolveNumber:telephoneNumber error:&error]) {
        ...
        [[YuloreAPIClient sharedClient] resolveTelenumberWithParameters:_parameters completionHandler:^(ResolveItemNew *resolveItem, NSError *error) {
            if (resolveItem ) {
...
- (void)setTelephoneNumber:(NSString *)telephoneNumber {
  ...
  _parameters[@"sig"] = [SignatureHelper resolveSignature:telephoneNumber];
