...
typedef void(^DHBSDKYuloreZipArchiveProgressUpdateBlock)(int percentage, int filesProcessed, int numFiles);
...
@protocol DHBSDKYuloreZipArchiveDelegate <NSObject>
@optional

/**
    @brief   Delegate method to be notified of errors
    
    YuloreZipArchive calls this selector on the delegate when errors are encountered.
...
@interface DHBSDKYuloreZipArchive : NSObject {
@private
	void*		_zipFile;
	void*		_unzFile;
  ...
  int  _numFiles;
	NSString*   _password;
	id			_delegate;
    DHBSDKYuloreZipArchiveProgressUpdateBlock _progressBlock;
...
/** a delegate object conforming to DHBSDKYuloreZipArchiveDelegate protocol */
@property (nonatomic, retain) id<DHBSDKYuloreZipArchiveDelegate> delegate;
@property (nonatomic, readonly)  int numFiles;
@property (nonatomic, copy) DHBSDKYuloreZipArchiveProgressUpdateBlock progressBlock;
