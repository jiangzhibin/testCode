...
typedef void(^YuloreZipArchiveProgressUpdateBlock)(int percentage, int filesProcessed, int numFiles);
...
@protocol YuloreZipArchiveDelegate <NSObject>
@optional
...
@interface YuloreZipArchive : NSObject {
@private
	void*		_zipFile;
	void*		_unzFile;
  ...
  int  _numFiles;
	NSString*   _password;
	id			_delegate;
    YuloreZipArchiveProgressUpdateBlock _progressBlock;
...
/** a delegate object conforming to YuloreZipArchiveDelegate protocol */
@property (nonatomic, retain) id<YuloreZipArchiveDelegate> delegate;
@property (nonatomic, readonly)  int numFiles;
@property (nonatomic, copy) YuloreZipArchiveProgressUpdateBlock progressBlock;
