...
@implementation DHBSDKYuloreZipArchive
...
{
	_zipFile = dhbsdk_zipOpen( (const char*)[zipFile UTF8String], 0 );
	if( !_zipFile ) 
		return NO;
	return YES;
}
...
//	tm_zip filetime;
	...
	{
		ret = dhbsdk_zipOpenNewFileInZip( _zipFile,
	...
	else
	{
		...
		ret = dhbsdk_zipOpenNewFileInZip3( _zipFile,
								  (const char*) [newname UTF8String],
								  &zipInfo,
								  NULL,0,
								  NULL,0,
	...
	ret = dhbsdk_zipWriteInFileInZip( _zipFile, (const void*)[data bytes], dataLen);
	if( ret!=Z_OK )
	{
		return NO;
	}
	ret = dhbsdk_zipCloseFileInZip( _zipFile );
	if( ret!=Z_OK )
		return NO;
	return YES;
}
...
-(BOOL) CloseZipFile2
{
	...
	if( _zipFile==NULL )
		return NO;
	BOOL ret =  dhbsdk_zipClose( _zipFile,NULL )==Z_OK?YES:NO;
	_zipFile = NULL;
	return ret;
}
...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	_unzFile = dhbsdk_unzOpen( (const char*)[zipFile UTF8String] );
	if( _unzFile )
	{
		dhbsdk_unz_global_info  globalInfo = {0};
		if( dhbsdk_unzGetGlobalInfo(_unzFile, &globalInfo )==UNZ_OK )
		{
            _numFiles = (int)globalInfo.number_entry;
			DHBSDKDLog(@"%lu entries in the zip file", globalInfo.number_entry);
		}
	}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
    ...
    int index = 0;
    int progress = -1;
	int ret = dhbsdk_unzGoToFirstFile( _unzFile );
	...
	do{
        @autoreleasepool {
            if( [_password length]==0 )
                ret = dhbsdk_unzOpenCurrentFile( _unzFile );
            else
                ret = dhbsdk_unzOpenCurrentFilePassword( _unzFile, password );
            if( ret!=UNZ_OK )
            {
            ...
            int read ;
            dhbsdk_unz_file_info	fileInfo ={0};
            ret = dhbsdk_unzGetCurrentFileInfo(_unzFile, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if( ret!=UNZ_OK )
            {
                [self OutputErrorMessage:@"Error occurs while getting file info"];
                success = NO;
                dhbsdk_unzCloseCurrentFile( _unzFile );
                break;
            }
            char* filename = (char*) malloc( fileInfo.size_filename +1 );
            dhbsdk_unzGetCurrentFileInfo(_unzFile, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
            ...
            do
            {
                read = dhbsdk_unzReadCurrentFile(_unzFile, buffer, 4096);
                if (read >= 0)
                {
                    if (fp == NULL) {
                ...
                if( fileInfo.dosDate!=0 )
                {
                    ...
                    if( attr )
                    {
                        ...
                        if( ![_fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] )
                        {
                            ...
                            DHBSDKDLog(@"Failed to set attributes");
                        }
            ...
            if (ret == UNZ_OK) {
                ret = dhbsdk_unzCloseCurrentFile( _unzFile );
                if (ret != UNZ_OK) {
            ...
            if (ret == UNZ_OK) {
                ret = dhbsdk_unzGoToNextFile( _unzFile );
            }
...
-(BOOL) UnzipCloseFile
{
	self.password = nil;
	if( _unzFile ) {
		int err = dhbsdk_unzClose( _unzFile );
        _unzFile = nil;
        return err ==UNZ_OK;
    }
	return YES;
...
@implementation NSFileManager(YuloreZipArchive)
