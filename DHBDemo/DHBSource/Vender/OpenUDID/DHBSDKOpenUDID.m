...
#define DHBSDKOpenUDIDLog(fmt, ...)
//#define DHBSDKOpenUDIDLog(fmt, ...) NSLog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define DHBSDKOpenUDIDLog(fmt, ...) NSLog((@"[Line %d] " fmt), __LINE__, ##__VA_ARGS__);
...
@interface DHBSDKOpenUDID (Private)
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
    ...
    if (item) {
        @try{
            ...
            DHBSDKOpenUDIDLog(@"Unable to unarchive item %@ on pasteboard!", [pboard name]);
            item = nil;
        }
    }
...
//
+ (NSString*) value {
    return [DHBSDKOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        DHBSDKOpenUDIDLog(@"localDict = %@",localDict);
    }
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [DHBSDKOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    //
    DHBSDKOpenUDIDLog(@"Available Slot %@ Existing Slot %@",availableSlotPBid,myRedundancySlotPBid);
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [DHBSDKOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
    ...
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:kOpenUDIDDomain
                                                     code:kDHBSDKOpenUDIDErrorOptedOut
    ...
    if (error!=nil) {
        if (isCompromised)
            *error = [NSError errorWithDomain:kOpenUDIDDomain
                                         code:kDHBSDKOpenUDIDErrorCompromised
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    DHBSDKOpenUDIDLog(@"Local dict after opt-out = %@",dict);
