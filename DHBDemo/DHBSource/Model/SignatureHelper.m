...
+ (NSString *) listPassword {
  NSString *sdkKey = [YuloreApiManager sharedYuloreApiManager].signature;
...
//  q = [q stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
 ...
 // NSString *dis_id =[info valueForKey:kDIS_ID] ? [info valueForKey:kDIS_ID] : @"";
  ...
  for (int i = 0; i < [locations count]; i++) {
    ...
    //  DLog(@"%@", [secretKEY substringWithRange:NSMakeRange(loc , len)]);
...
+ (NSString *) detialSignature:(NSDictionary *)info {
  ...
  for (int i = 0; i < [locations count]; i++) {
    NSUInteger loc = [locations[i] integerValue];
    NSUInteger len = [lengths[i] integerValue];
    //  DLog(@"%@", [new substringWithRange:NSMakeRange(loc , len)]);
...
+ (NSString *) signatureWithDictionary:(NSMutableDictionary *)info {
  ...
  switch (type) {
    case SIGNATURETYPELIST:
      SHA1String = [self listSignature:info];
      break;
    case SIGNATURETYPEDETIAL:
      SHA1String = [self detialSignature:info];
      break;
    case SIGNATURETYPECATEGORY:
...
+ (NSString *) batchResolveSignature:(NSString *)telenumberString {
  ...
  NSString *string = [NSString stringWithFormat:@"vs2#D_%@d&-vaSc%@7szV%@s!jc%@Cs5$N%@", uid,[YuloreApiManager sharedYuloreApiManager].apiKey, uid,telenumberString, [YuloreApiManager sharedYuloreApiManager].apiKey];
...
+ (NSString *) resolveSignature:(NSString *)telenumber {
  ...
  NSString *sig = [YuloreApiManager sharedYuloreApiManager].signature;
  ...
  NSString *string = [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@",str1,uid,str2,telenumber,str3,uid,str4,[YuloreApiManager sharedYuloreApiManager].apiKey,str5,telenumber];
...
+ (NSString *) flagSignature:(NSString *)telenumber withFlag:(NSString *)flag withAppname:(NSString *)appName{
  ...
  NSString *str7 = [sig substringWithRange:r7];
    ...
    NSString *string = [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@%@%@%@",str1, telenumber,str2, appName ,str3, flag,str4, telenumber,str5,  [YuloreApiManager sharedYuloreApiManager].apiKey,str6, uid, str7];
  
  
//  NSString *app = @"com.yulore.yellowpage";
  //NSString *string = [NSString stringWithFormat:@"j*sD5&_%@ds2{%@hX13%@e2@s%@9C#s3#%@zF!v%%b%@a^2Dc", telenumber, app,flag,telenumber, [YuloreApiManager sharedYuloreApiManager].apiKey, uid];
  NSString *sha1String = [string sha1String];
...
+ (NSString *)orderSignature:(NSString *)parametersString {
  NSString *APISECRET = [YuloreApiManager sharedYuloreApiManager].signature;
