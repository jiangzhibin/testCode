...
+ (instancetype)shopItemWithResolveItem:(ResolveItemNew *)aResolveItem {
  ShopItem *aShopItem = [[ShopItem alloc] init];
  if (aShopItem) {
    ...
    if (aResolveItem.teleNumbers && [aResolveItem.teleNumbers count] > 0) {
      aShopItem.telenumber =((TeleNumber *) aShopItem.teleNumbers.firstObject).teleNumber;
    }
    else {
      TeleNumber *aTel = [[TeleNumber alloc] init];
...
+ (instancetype)shopItemWithDictionary:(NSDictionary *)dictionary {
  ShopItem *shopItem = [[ShopItem alloc] initWithDictionary:dictionary];
...
- (id)initWithDictionary:(NSDictionary *)item {
 ...
 //   _tuan = [item valueForKey:@"tuan"] ? YES : NO;
    ...
    for (NSDictionary *aTel in telnumbersArray) {
      TeleNumber *aTelNumber = [[TeleNumber alloc] initWithDictionary:aTel];
      [_teleNumbers addObject:aTelNumber];
    }
    
    
    TeleNumber *disp = [_teleNumbers count] > 0 ? [_teleNumbers objectAtIndex:0] : [[TeleNumber alloc] initWithBlank];
    ...
    for (NSDictionary *aCustom in customsArray) {
      CustomItem *aCus = [[CustomItem alloc] initWithDictionary:aCustom];
